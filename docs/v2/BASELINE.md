...
| WP-12 | **accepted** (coordinator-executed final verification: suite + strict build + fixture regeneration + docs) | gate worker (WP12Gate) + coordinator | `0bf7827`/`f1af8a5` (folded WP-09 gate fixes) | 12.1–12.5 | `docs/v2/VERIFICATION.md` (§10 matrix, per-row evidence/deferrals, B01–B17→test map), `docs/v2/PERFORMANCE.md` (measured core paths: save 36.86ms / load 1.50ms / record 0.029ms / decode 0.0053ms; soak = blocked, needs packaged app), `docs/v2/RELEASE_CHECKLIST.md` (user-eval protocol, soak protocol, distribution), README rewrite (585-test badge, v2 features/semantics), CHANGELOG 2.0 UNRELEASED entry, Makefile suite counter fixed (53), 500-cycle lifecycle harness stable (34 subviews), WP-09 shell wiring (history menu item) | 585 tests / 53 suites EXIT=0 on merged `v2.0` @ `2fa2663` (`/tmp/v2-final-everything2.log`); strict build exit 0; `make build` exit 0; 46 fixture PNGs regenerated | — | Release acts (version bump, tag, publish, notarization, user evaluation, app-level soak) = coordinator+user authorization per RELEASE_CHECKLIST.md | — |
| WP-09 | **accepted** (coordinator diff-review + suite evidence) | features worker (WP09Markers) | `ddf2db0` | 09.1–09.3 | `SpendMarker` (comparable-delta rule: scope/day discontinuity and non-finite → explicit unavailable, never guessed; offline same-day recovery with caveat; ≤80-char name), `HistoryExport` (RFC 4180 + formula-injection-safe CSV, locale-independent decimals, no tokens), `HistoryRepository` marker store + marker-boundary pinning in coalescer/cap, `HistoryWindowController`/`HistoryDayView` (lazy window, reopen restores selection, legacy archive separate), `MarkerView` | 575 tests / 53 suites on `v2.0` (45 new: 10→25=$15, restart, invalid end, stale boundaries, midnight, token replacement, CSV round-trip, injection safety); strict build exit 0, zero new warnings | — | Shell wiring (menu item + popover marker buttons) proposed to coordinator — WP-10's SecondaryPanelCoordinator seam is the anchor | WP-12 |
...
coalescing/cap; 575/53 EXIT=0 post-merge). WP-10 **accepted** (diff review: AX press actions route through the same
